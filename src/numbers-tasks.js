...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sklrpoisv = x1 * x2 + y1 * y2;
  const module1 = (x1 ** 2 + y1 ** 2) ** (1 / 2);
  const module2 = (x2 ** 2 + y2 ** 2) ** (1 / 2);
  let cosin = sklrpoisv / (module1 * module2);
  cosin = Math.max(-1, Math.min(1, cosin));
...
function getCountOfOddNumbers(number) {
  ...
  let num = 0;
  for (let i = 1; i <= absoluteNumber; i += 2) {
    num += 1;
  }
  return num;
}
